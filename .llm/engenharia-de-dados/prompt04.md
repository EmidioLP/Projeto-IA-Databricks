...
para a sala: _"quantas pessoas na sua empresa escreveriam esse `WHERE NOT
p.cancelado`? E quantas esqueceriam?"_ Cada esquecimento é um relatório com
...
> _"Essa regra de margem — receita menos custo, sem frete, sem desconto
...
> numa coluna com COMMENT, e valer para a empresa inteira."_
...
- **O contrato vem antes do SQL.** Granularidade, dimensões, métricas e filtros
  definidos **antes** da primeira linha. `fato_vendas` tem grão de _item de
  pedido_ — escrever isso numa frase evita seis meses de discussão.
...
> _"Quatro tabelas diferentes, quatro consumidores diferentes, o mesmo número.
> É isso que a palavra 'conformado' significa — e é o teste 8."_
...
> _"O dashboard vai ficar com o dado de ontem. É de longe o melhor dos dois
> cenários ruins — o outro é ele ficar com o dado errado de hoje, e ninguém
> perceber até a reunião de segunda."_

---

## Fala de aula

> _"Ontem eu levei quinze minutos escrevendo query para chegar nesse número.
...
> está escrita numa tabela e não na cabeça de quem escreveu a query._
>
> _E olha o último bloco: nove testes. O primeiro é o que mais importa — a
...
| ---------------------------------------------- | -------------------------------------- | --------------------------------------------------------------------- |
| A gold soma R$ 103,6 mi e a silver R$ 102,3 mi | A devolução ficou de fora do fato      | _"Traga a devolução para dentro do fato, com valor negativo e flag."_ |
